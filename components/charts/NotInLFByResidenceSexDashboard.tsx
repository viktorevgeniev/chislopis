...
export function NotInLFByResidenceSexDashboard({ data, dataset, locale = 'en' }: DashboardProps) {
  ...
  const kpiData = useMemo(() => {
    ...
    return { latestQ, totalVal, yoyChange, urbanVal, ruralVal, urbanPct, femalePct };
  }, [allQuarters, lookup]);
  ...
  return (
    <Card className="bg-slate-50 border-0 shadow-none">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Chart B: Stacked area chart */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? 'Б. Структура във времето' : 'B. Composition Over Time'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg ? 'Градско/Селско или Мъже/Жени като дял от общото' : 'Urban/Rural or Male/Female proportional composition'}
          </p>
          <StackedAreaChart allQuarters={allQuarters} getValue={getValue} locale={locale} />
        </div>

        {/* Chart C: Grouped bar chart — latest quarter */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? `В. Демографска снимка (${kpiData.latestQ})` : `C. Demographic Snapshot (${kpiData.latestQ})`}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
...
function StackedAreaChart({ allQuarters, getValue, locale }: {
  allQuarters: string[];
  getValue: (q: string, res: string, gen: string) => number | null;
  locale: 'bg' | 'en';
}) {
  const chartRef = useRef<HTMLDivElement>(null);
  const isBg = locale === 'bg';
  const [stackBy, setStackBy] = useState<'residence' | 'sex'>('residence');
  ...
  useEffect(() => {
    ...
    if (stackBy === 'residence') {
      legendData = [urbanLabel, ruralLabel];
      series = [
        {
          name: urbanLabel, type: 'line', stack: 'total', areaStyle: { opacity: 0.55 },
          data: allQuarters.map(q => getValue(q, '1', '0')),
          itemStyle: { color: COLORS.urban }, lineStyle: { width: 1.5 }, smooth: true, symbol: 'none',
          emphasis: { focus: 'series' as const },
        },
        {
          name: ruralLabel, type: 'line', stack: 'total', areaStyle: { opacity: 0.55 },
          data: allQuarters.map(q => getValue(q, '2', '0')),
          itemStyle: { color: COLORS.rural }, lineStyle: { width: 1.5 }, smooth: true, symbol: 'none',
          emphasis: { focus: 'series' as const },
        },
      ];
    } else {
    ...
    window.addEventListener('resize', h);
    return () => { window.removeEventListener('resize', h); chart.dispose(); };
  }, [allQuarters, getValue, stackBy, isBg]);

  return (
    <div>
      <div className="flex gap-2 mb-3">
        {([
          ['residence', isBg ? 'Градско / Селско' : 'Urban / Rural'],
          ['sex', isBg ? 'Мъже / Жени' : 'Male / Female'],
        ] as const).map(([code, label]) => (
          <button
            key={code}
            onClick={() => setStackBy(code as 'residence' | 'sex')}
            className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${
              stackBy === code
                ? 'bg-slate-800 text-white'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
...
// ── Chart C: Grouped Bar — Latest Quarter ───────────────────────────────────────
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          params.forEach((p: any) => {
            ...
            const pctTotal = grandTotal > 0 ? (val / grandTotal * 100).toFixed(1) : '0';
            tip += `<div style="display:flex;gap:6px;margin:1px 0">
              <span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${p.color};margin-top:3px"></span>
              <span style="flex:1">${p.seriesName}</span>
              <span style="font-weight:600">${val.toFixed(1)} ${isBg ? 'хил.' : 'K'} (${pctGroup}%)</span>
            </div>`;
          });
