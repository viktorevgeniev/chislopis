...
export function TeachingStaffKindergartensDashboard({ data, locale = 'en' }: Props) {
      ...
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-semibold text-slate-900">
          {isBg
            ...
            : 'Teaching Staff in Kindergartens by Districts and Municipalities'}
        </CardTitle>
      ...
      <CardContent className="space-y-6">
        ...
        {kpi.total != null && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <KpiCard
              label={isBg ? 'Педагогически персонал (общо)' : 'Teaching Staff (total)'}
...
type Accent = 'teal' | 'blue' | 'green' | 'amber';
    ...
    <div className="bg-white shadow-sm rounded-xl p-4 mt-3 space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-400 mb-3">
          {isBg
            ? `Общ брой педагогически персонал в детски градини в България (${firstYear}–${latestYear})`
            : `Total teaching staff in kindergartens in Bulgaria (${firstYear}–${latestYear})`}
        </p>
...
function NationalTrendChart({
  ...
  useEffect(() => {
    ...
    const option: EChartsOption = {
      ...
      series: [{
        name: isBg ? 'Педагогически персонал' : 'Teaching Staff',
        type: 'line' as const,
        data: values,
        ...
        areaStyle: {
          color: {
            ...
            x: 0, y: 0, x2: 0, y2: 1,
            colorStops: [
              { offset: 0, color: '#0d948824' },
              { offset: 1, color: '#0d948804' },
            ],
          },
        },
      }],
...
function RegionsSection({
  ...
  return (
    <div className="bg-white shadow-sm rounded-xl p-4 mt-3">
      ...
      <p className="text-xs text-slate-400 mb-3">
        {isBg
          ? `Брой педагогически персонал по региони за ${selectedYear}${nationalTotal != null ? ` | Общо: ${fmtNum(nationalTotal)}` : ''}`
          : `Teaching staff by region for ${selectedYear}${nationalTotal != null ? ` | Total: ${fmtNum(nationalTotal)}` : ''}`}
      </p>
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<div style="font-weight:600;color:#0f172a;margin-bottom:4px">${p.name}</div>
            <div>${isBg ? 'Персонал' : 'Staff'}: <b>${fmtNum(p.value)}</b>${pct ? ` (${pct}% ${isBg ? 'от общото' : 'of total'})` : ''}</div>`;
        },
      },
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          const pct = nationalTotal && nationalTotal > 0
            ? ((p.value / nationalTotal) * 100).toFixed(1) : null;
          return `<div style="font-weight:600;color:#0f172a;margin-bottom:4px">${p.name}</div>
            <div>${isBg ? 'Персонал' : 'Staff'}: <b>${fmtNum(p.value)}</b>${pct ? ` (${pct}%)` : ''}</div>`;
        },
      },
      ...
      <div className="flex flex-wrap gap-3 items-end mb-4">
        ...
        <div className="flex items-center gap-1 flex-1 min-w-40">
          ...
          <input
            ...
            className="flex-1 text-xs border border-slate-200 rounded px-2 py-1 h-8 focus:outline-none focus:ring-1 focus:ring-teal-300"
          />
        </div>
        ...
        <table className="w-full text-xs text-slate-600 border-collapse">
          <thead>
            <tr className="border-b-2 border-slate-100 text-[11px] text-slate-400 uppercase">
              ...
              >
                {isBg ? 'Персонал' : 'Staff'} <SortIcon k="value" />
              </th>
              <th
              ...
              pageRows.map((r: any, i) => {
                ...
                return (
                  <tr key={`${r.code}-${r.year}`} className={`border-b border-slate-50 hover:bg-slate-50 ${isDistrict ? 'font-medium' : ''}`}>
                    ...
                    <td className="py-1.5 pr-3">
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${isDistrict ? 'bg-teal-50 text-teal-600' : 'bg-slate-100 text-slate-500'}`}>
                        {isDistrict
                          ? (isBg ? 'Обл.' : 'District')
                          : (isBg ? 'Общ.' : 'Mun.')}
                      </span>
                    </td>
                    <td className="py-1.5 pr-3 text-right font-medium text-teal-600">
                      {fmtNum(r.value)}
                    </td>
