...
// ── Bar chart categories (Chart B) — curated selection ────────────────────────

const BAR_CODES = ['12', '5', '15', '6', '1', '9', '11', '0', '7', '14', '13', '3', '20', '8', '10', '18'];

// ── Broad categories for clustered column chart (Chart C) ─────────────────────

const CLUSTER_GROUPS = [
  { code: '12', colorEn: '#ef4444', colorBg: '#ef4444' },
  { code: '11', colorEn: '#f59e0b', colorBg: '#f59e0b' },
  { code: '3',  colorEn: '#10b981', colorBg: '#10b981' },
  { code: '18', colorEn: '#0284c7', colorBg: '#0284c7' },
];
...
export function RiskPovertyExclusionHouseholdDashboard({ data, locale = 'en' }: Props) {
  ...
  // Last 5 available years for Chart C
  ...
  const kpi = useMemo(() => {
    ...
    const totalLatest     = getVal(findRate('0',  latestYear), 'Rate');
    const singleParent    = getVal(findRate('12', latestYear), 'Rate');
    const twoAdults3Plus  = getVal(findRate('15', latestYear), 'Rate');
    const twoAdultsLatest = getVal(findRate('20', latestYear), 'Rate');
    const totalPrev       = prevYear ? getVal(findRate('0', prevYear), 'Rate') : null;
    const totalChange     = totalLatest != null && totalPrev != null ? totalLatest - totalPrev : null;

    return { totalLatest, singleParent, twoAdults3Plus, twoAdultsLatest, totalChange, latestYear };
    ...
    <Card className="bg-slate-50 border-0 shadow-none">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            ...
            <p className="text-3xl font-bold mt-2 text-slate-700">
              {kpi.totalLatest != null ? `${kpi.totalLatest.toFixed(1)}%` : '—'}
            </p>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] text-slate-400">{kpi.latestYear}</span>
              {kpi.totalChange != null && (
                <span className={`text-[10px] font-semibold ${kpi.totalChange <= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                  {kpi.totalChange <= 0 ? '▼' : '▲'} {Math.abs(kpi.totalChange).toFixed(1)}pp
                </span>
              )}
            </div>
          </div>
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              {isBg ? 'Единичен родител' : 'Single parent'}
            </p>
            <p className="text-3xl font-bold mt-2 text-red-600">
              {kpi.singleParent != null ? `${kpi.singleParent.toFixed(1)}%` : '—'}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {isBg ? 'С зависими деца' : 'With dependent children'}
            </p>
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              {isBg ? 'Двама + 3+ деца' : 'Two adults + 3+ children'}
            </p>
            <p className="text-3xl font-bold mt-2 text-orange-600">
              {kpi.twoAdults3Plus != null ? `${kpi.twoAdults3Plus.toFixed(1)}%` : '—'}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {isBg ? 'Домакинства с много деца' : 'Large family households'}
            </p>
            <p className="text-[10px] text-slate-400 mt-1">{kpi.latestYear}</p>
          </div>
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            ...
            <p className="text-3xl font-bold mt-2 text-blue-600">
              {kpi.twoAdultsLatest != null ? `${kpi.twoAdultsLatest.toFixed(1)}%` : '—'}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {isBg ? 'Ниска уязвимост' : 'Lower vulnerability'}
            </p>
            <p className="text-[10px] text-slate-400 mt-1">{kpi.latestYear}</p>
          </div>
        </div>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg
              ...
              : 'A. Poverty Risk Trend by Household Type'}
          </h3>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg
              ? `Б. Риск от бедност по тип домакинство — ${latestYear}`
              : `B. Poverty Risk by Household Type — ${latestYear}`}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Дял от населениeто в риск — наредено по стойност'
              : 'Share of population at risk — ranked by value'}
          </p>
          <HouseholdBarChart data={data} latestYear={latestYear} locale={locale} />
        </div>

        {/* ── Chart C: Clustered column — broad categories, last 5 years ── */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg
              ? `В. Сравнение на широки категории домакинства — ${last5Years[0]}–${last5Years[last5Years.length - 1]}`
              : `C. Broad Household Categories Comparison — ${last5Years[0]}–${last5Years[last5Years.length - 1]}`}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Групирани стълбове за четири широки категории домакинства'
              : 'Clustered bars for four broad household categories'}
          </p>
          <ClusteredColumnChart data={data} years={last5Years} locale={locale} />
        </div>
...
function HouseholdBarChart({ data, latestYear, locale }: {
  ...
  return <div ref={chartRef} style={{ width: '100%', height: `${Math.max(280, categories.length * 34)}px` }} />;
}

// ══════════════════════════════════════════════════════════════════════════════
// Chart C — Clustered Column: broad categories over last 5 years
// ══════════════════════════════════════════════════════════════════════════════

function ClusteredColumnChart({ data, years, locale }: {
  data: any[];
  years: string[];
  locale: 'bg' | 'en';
}) {
  ...
  const seriesData = useMemo(() => {
    return CLUSTER_GROUPS.map(({ code, colorEn }) => ({
      label: hhLabel(code, isBg),
      color: colorEn,
      values: years.map(year => {
        const row = data.find(d =>
          String(d.SILC_HHType_Code) === code && String(d.Year) === year
        );
  ...
  useEffect(() => {
    ...
    const option: EChartsOption = {
      ...
      series: seriesData.map(s => ({
        ...
        type: 'bar' as const,
        data: s.values,
        itemStyle: { color: s.color, borderRadius: [4, 4, 0, 0], opacity: 0.88 },
        barMaxWidth: 56,
        label: {
          show: true,
