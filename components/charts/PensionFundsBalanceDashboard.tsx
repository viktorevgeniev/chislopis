...
// ── Indicator codes (Balance Sheet, NSI ID 781 – Supplementary Pension Funds) ─
const C = {
  // Assets
  TOTAL_ASSETS:    '3.1.2.4.3.0.1',
  INVESTMENTS:     '3.1.2.4.3.0.1.1',
  FA_BG:           '3.1.2.4.3.0.1.1.1',
  FA_BG_EQUITY:    '3.1.2.4.3.0.1.1.1.1',
  FA_BG_DEBT:      '3.1.2.4.3.0.1.1.1.2',
  FA_BG_HEDGE:     '3.1.2.4.3.0.1.1.1.3',
  FA_FOREIGN:      '3.1.2.4.3.0.1.1.2',
  FA_FOR_EQUITY:   '3.1.2.4.3.0.1.1.2.1',
  FA_FOR_DEBT:     '3.1.2.4.3.0.1.1.2.2',
  FA_FOR_HEDGE:    '3.1.2.4.3.0.1.1.2.3',
  DEPOSITS:        '3.1.2.4.3.0.1.1.3',
  INV_ESTATE:      '3.1.2.4.3.0.1.1.4',
  CASH:            '3.1.2.4.3.0.1.2',
  RECEIVABLES:     '3.1.2.4.3.0.1.3',
  RECV_INVEST:     '3.1.2.4.3.0.1.3.1',
  // Liabilities
  TOTAL_LIAB:      '3.1.2.4.3.0.2',
  RESERVE_MIN:     '3.1.2.4.3.0.2.1',
  NC_LIAB:         '3.1.2.4.3.0.2.2',
  NC_LIAB_INSURED: '3.1.2.4.3.0.2.2.1',
  NC_LIAB_PENSION: '3.1.2.4.3.0.2.2.2',
  C_LIAB:          '3.1.2.4.3.0.2.3',
  C_LIAB_INVEST:   '3.1.2.4.3.0.2.3.1',
} as const;

const LABEL_EN: Record<string, string> = {
  [C.TOTAL_ASSETS]:    'Total Assets',
  [C.INVESTMENTS]:     'Investments',
  [C.FA_BG]:           'Financial Assets (Bulgaria)',
  [C.FA_BG_EQUITY]:    'Equity Instruments (BG)',
  [C.FA_BG_DEBT]:      'Debt Instruments (BG)',
  [C.FA_BG_HEDGE]:     'Hedging Instruments (BG)',
  [C.FA_FOREIGN]:      'Financial Assets (Foreign)',
  [C.FA_FOR_EQUITY]:   'Equity Instruments (Foreign)',
  [C.FA_FOR_DEBT]:     'Debt Instruments (Foreign)',
  [C.FA_FOR_HEDGE]:    'Hedging Instruments (Foreign)',
  [C.DEPOSITS]:        'Deposits in Banks',
  [C.INV_ESTATE]:      'Investment Estate',
  [C.CASH]:            'Cash',
  [C.RECEIVABLES]:     'Current Receivables',
  [C.RECV_INVEST]:     'Receivables (Investments)',
  [C.TOTAL_LIAB]:      'Total Liabilities',
  [C.RESERVE_MIN]:     'Min Profitability Reserve',
  [C.NC_LIAB]:         'Non-current Liabilities',
  [C.NC_LIAB_INSURED]: 'NC Liab. to Insured Persons',
  [C.NC_LIAB_PENSION]: 'NC Liab. to Pensioners',
  [C.C_LIAB]:          'Current Liabilities',
  [C.C_LIAB_INVEST]:   'Current Liab. (Investments)',
};

const LABEL_BG: Record<string, string> = {
  [C.TOTAL_ASSETS]:    'Активи общо',
  [C.INVESTMENTS]:     'Инвестиции',
  [C.FA_BG]:           'Фин. активи (България)',
  [C.FA_BG_EQUITY]:    'Дялови инстр. (БГ)',
  [C.FA_BG_DEBT]:      'Дългови инстр. (БГ)',
  [C.FA_BG_HEDGE]:     'Хеджиращи инстр. (БГ)',
  [C.FA_FOREIGN]:      'Фин. активи (чужбина)',
  [C.FA_FOR_EQUITY]:   'Дялови инстр. (чужд.)',
  [C.FA_FOR_DEBT]:     'Дългови инстр. (чужд.)',
  [C.FA_FOR_HEDGE]:    'Хеджиращи инстр. (чужд.)',
  [C.DEPOSITS]:        'Депозити в банки',
  [C.INV_ESTATE]:      'Инвестиционни имоти',
  [C.CASH]:            'Парични средства',
  [C.RECEIVABLES]:     'Краткосрочни вземания',
  [C.RECV_INVEST]:     'Вземания (инвестиции)',
  [C.TOTAL_LIAB]:      'Пасиви общо',
  [C.RESERVE_MIN]:     'Резерв за мин. доходност',
  [C.NC_LIAB]:         'Дълг. задължения',
  [C.NC_LIAB_INSURED]: 'Дълг. задълж. към осигурени',
  [C.NC_LIAB_PENSION]: 'Дълг. задълж. към пенсионери',
  [C.C_LIAB]:          'Краткоср. задължения',
  [C.C_LIAB_INVEST]:   'Краткоср. задълж. (инв.)',
};
...
// Main Dashboard
// ══════════════════════════════════════════════════════════════════════════════
export function PensionFundsBalanceDashboard({ data, locale = 'en' }: Props) {
  ...
  const kpi = useMemo(() => {
    ...
    return {
      ...
      investments:  yd?.get(C.INVESTMENTS)   ?? null,
    ...
    <Card className="bg-slate-50 border-0 shadow-none">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-semibold text-slate-900">
          {isBg
            ...
            : 'Supplementary Pension Insurance Funds — Balance Sheet'}
        </CardTitle>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          />
          <KPICard
            label={isBg ? 'Инвестиции' : 'Investments'}
            value={kpi.investments}
            year={latestYear}
            color="text-violet-600"
            isBg={isBg}
          />
          <KPICard
            label={isBg ? 'Пасиви общо' : 'Total Liabilities'}
            value={kpi.totalLiab}
            year={latestYear}
            color="text-rose-600"
            isBg={isBg}
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? 'А. Динамика на баланса' : 'A. Balance Sheet Growth Trend'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Активи общо vs. Пасиви общо | хиляди лв.'
              : 'Total Assets vs. Total Liabilities | Thousand BGN'}
          </p>
          <MacroTrendChart dataMap={dataMap} years={allYears} isBg={isBg} label={label} />
        </div>

        {/* ── Chart B: Asset Composition Donut ── */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg
              ? `Б. Структура на активите (${latestYear})`
              : `B. Asset Composition (${latestYear})`}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Инвестиции · Парични средства · Краткосрочни вземания | хиляди лв.'
              : 'Investments · Cash · Current Receivables | Thousand BGN'}
          </p>
          <AssetCompositionChart dataMap={dataMap} latestYear={latestYear} isBg={isBg} label={label} />
        </div>

        {/* ── Chart C: Investment Breakdown Grouped Bar ── */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? 'В. Структура на инвестициите' : 'C. Investment Breakdown Over Time'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Дялови инстр. (БГ/чужд.) · Дългови инстр. (БГ/чужд.) · Депозити | хиляди лв.'
              : 'Equity (BG/Foreign) · Debt (BG/Foreign) · Deposits in Banks | Thousand BGN'}
          </p>
          <InvestmentBreakdownChart dataMap={dataMap} years={allYears} isBg={isBg} label={label} />
        </div>

        {/* ── Chart D: Liabilities Structure Stacked Bar ── */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? 'Г. Структура на пасивите' : 'D. Liabilities Structure'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? 'Резерв за мин. доходност · Дълг. задълж. · Краткоср. задълж. | хиляди лв.'
              : 'Min Profitability Reserve · Non-current Liabilities · Current Liabilities | Thousand BGN'}
          </p>
          <LiabilitiesStructureChart dataMap={dataMap} years={allYears} isBg={isBg} label={label} />
        </div>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            <div className="flex flex-col gap-1">
              ...
              <select
                className="text-sm border border-slate-200 rounded-lg px-3 py-1.5 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-300 min-w-[220px]"
...
// ══════════════════════════════════════════════════════════════════════════════
// Chart A — Macro Trend: Total Assets vs Total Liabilities (multi-line + area)
// ══════════════════════════════════════════════════════════════════════════════
function MacroTrendChart({ dataMap, years, isBg, label }: {
  ...
  useEffect(() => {
      ...
      ...(areaColor ? {
        areaStyle: {
          color: {
            ...
            colorStops: [
              { offset: 0, color: areaColor.replace(')', ',0.15)').replace('rgb', 'rgba') },
    ...
    const option: EChartsOption = {
      ...
      series: [
        makeLine(label(C.TOTAL_ASSETS), series.assets, COLORS.totalAssets, 'rgb(59,130,246)'),
        makeLine(label(C.TOTAL_LIAB),   series.liab,   COLORS.totalLiab,   'rgb(239,68,68)'),
      ],
    };
...
// ══════════════════════════════════════════════════════════════════════════════
// Chart B — Asset Composition Donut (latest year)
// ══════════════════════════════════════════════════════════════════════════════
function AssetCompositionChart({ dataMap, latestYear, isBg, label }: {
...
// ══════════════════════════════════════════════════════════════════════════════
// Chart C — Investment Breakdown: Grouped Bar (Equity/Debt BG + Foreign + Deposits)
// ══════════════════════════════════════════════════════════════════════════════
const INV_CODES = [
  C.FA_BG_EQUITY,
  C.FA_BG_DEBT,
...
const INV_COLORS: Record<string, string> = {
  [C.FA_BG_EQUITY]:  COLORS.bgEquity,
  [C.FA_BG_DEBT]:    COLORS.bgDebt,
  [C.FA_FOR_EQUITY]: COLORS.forEquity,
  [C.FA_FOR_DEBT]:   COLORS.forDebt,
  [C.DEPOSITS]:      COLORS.deposits,
};

function InvestmentBreakdownChart({ dataMap, years, isBg, label }: {
  dataMap: DataMap; years: string[]; isBg: boolean; label: (c: string) => string;
}) {
  const chartRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const barSeries = INV_CODES.map(code => ({
      name: label(code),
      type: 'bar' as const,
      data: years.map(y => {
      ...
      itemStyle: { color: INV_COLORS[code] },
      emphasis: { focus: 'series' as const },
    }));

    // Overlay: total investments line
    const totalLine = {
      name: label(C.INVESTMENTS),
      type: 'line' as const,
      data: years.map(y => dataMap.get(y)?.get(C.INVESTMENTS) ?? null),
    ...
    const option: EChartsOption = {
      ...
      legend: {
        data: [...INV_CODES.map(c => label(c)), label(C.INVESTMENTS)],
        bottom: '2%',
        textStyle: { fontSize: 10, color: '#64748b' },
        itemWidth: 12, itemHeight: 8,
      },
      grid: { left: '1%', right: '2%', bottom: '28%', top: '6%', containLabel: true },
      xAxis: {
        type: 'category',
        data: years,
        axisLabel: { fontSize: 10, color: '#94a3b8', rotate: years.length > 15 ? 30 : 0 },
...
// Chart D — Liabilities Structure: Stacked Bar
// ══════════════════════════════════════════════════════════════════════════════
const LIAB_CODES = [C.RESERVE_MIN, C.NC_LIAB, C.C_LIAB] as const;

const LIAB_COLORS: Record<string, string> = {
  [C.RESERVE_MIN]: COLORS.reserveMin,
  [C.NC_LIAB]:     COLORS.ncLiab,
  [C.C_LIAB]:      COLORS.cLiab,
};

function LiabilitiesStructureChart({ dataMap, years, isBg, label }: {
  dataMap: DataMap; years: string[]; isBg: boolean; label: (c: string) => string;
}) {
  const chartRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const barSeries = LIAB_CODES.map(code => ({
      ...
      itemStyle: {
        color: LIAB_COLORS[code],
        borderRadius: code === C.NC_LIAB ? [3, 3, 0, 0] : undefined,
      },
      emphasis: { focus: 'series' as const },
    }));
    ...
    const option: EChartsOption = {
      ...
      legend: {
        data: [...LIAB_CODES.map(c => label(c)), label(C.TOTAL_LIAB)],
        bottom: '2%',
        textStyle: { fontSize: 11, color: '#64748b' },
        itemWidth: 14, itemHeight: 8,
      },
