...
export function LabourForceByEducationDashboard({ data, dataset, locale = 'en' }: DashboardProps) {
  ...
  const kpiData = useMemo(() => {
    ...
    const higherRow = findRow(latestQ, '1', '0');
    const higherVal = higherRow ? getVal(higherRow) : null;
    ...
    return { latestQ, totalVal, yoyChange, higherPct, vocPct };
  }, [data, allQuarters]);
    ...
    <Card className="bg-slate-50 border-0 shadow-none">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              {isBg ? '% Висше образование' : '% Higher Education'}
            </p>
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg shadow-sm">
          <label htmlFor="lf-edu-age-filter" className="text-sm font-medium text-slate-600">
            {isBg ? 'Възрастова група:' : 'Age Group:'}
          </label>
          <Select id="lf-edu-age-filter" value={selectedAge} onChange={(e) => setSelectedAge(e.target.value)} className="w-[200px]">
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? 'А. Образователна структура на работната сила' : 'A. Labour Force by Education Level'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg ? 'Натрупани по образователно ниво във времето' : 'Stacked by education level over time'}
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white shadow-sm rounded-xl p-4">
            <h3 className="text-sm font-semibold text-slate-700 mb-1">
              {isBg ? 'В. Текущо разпределение по образование' : 'C. Current Labour Force by Education'}
            </h3>
            <p className="text-xs text-slate-400 mb-3">
