...
// Macro-region each district belongs to
...
export function MonetaryPovertyRateRegionalDashboard({ data, locale = 'en' }: Props) {
  const isBg = locale === 'bg';

  // Build index: nutsCode → year → rate
  const index = useMemo(() => {
    const map = new Map<string, Map<string, number>>();
    for (const row of data) {
      const code: string = row.NUTS_Code ?? '';
      const year: string = String(row.Year ?? '');
      const rate = getVal(row, 'Rate');
      if (!code || !year || rate == null) continue;
      if (!map.has(code)) map.set(code, new Map());
      map.get(code)!.set(year, rate);
    }
    return map;
  }, [data]);
  ...
  // KPI data
  const kpi = useMemo(() => {
    const nationalRate = index.get('BG')?.get(latestYear) ?? null;
    const nationalPrev = prevYear ? (index.get('BG')?.get(prevYear) ?? null) : null;
    const yoy = nationalRate != null && nationalPrev != null ? nationalRate - nationalPrev : null;
    ...
    <Card className="bg-slate-50 border-0 shadow-none">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-semibold text-slate-900">
          {isBg ? 'Парична бедност по региони (NUTS)' : 'Monetary Poverty Rate by Region (NUTS)'}
        </CardTitle>
        <CardDescription className="text-slate-500">
          {isBg
            ? `Годишни данни (${firstYear}–${latestYear}) | Дял на лицата под прага на бедност по статистически региони и области (% от населението)`
            : `Annual data (${firstYear}–${latestYear}) | Share of persons below the poverty threshold by statistical regions and districts (% of population)`}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            ...
            <p className="text-3xl font-bold mt-2 text-indigo-600">
              {kpi.nationalRate != null ? `${kpi.nationalRate.toFixed(1)}%` : '—'}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {isBg ? 'Общо за страната' : 'Total for the country'}
            </p>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg
              ? 'А. Динамика на паричната бедност по статистически региони'
              : 'A. Monetary Poverty Rate Trend by Statistical Region'}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? `${firstYear}–${latestYear} | Национален показател и макрорегиони`
              : `${firstYear}–${latestYear} | National indicator and macro-regions`}
          </p>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
            <h3 className="text-sm font-semibold text-slate-700">
              {isBg
                ? 'Б. Парична бедност по области — класация'
                : 'B. Monetary Poverty Rate by District — Ranking'}
            </h3>
            <Select
          ...
          <p className="text-xs text-slate-400 mb-3">
            {isBg
              ? `${selectedBarYear} | Подредени по намаляващ дял (само ниво NUTS 3)`
              : `${selectedBarYear} | Sorted by descending rate (NUTS 3 districts only)`}
          </p>
...
function TrendLineChart({ index, allYears, isBg }: {
  ...
  const seriesData = useMemo(() => (
    MACRO_CODES.map(code => ({
      ...
      isNational: code === 'BG',
      values: allYears.map(yr => index.get(code)?.get(yr) ?? null),
    }))
  ), [index, allYears, isBg]);
...
function RegionalBarChart({ index, year, isBg }: {
  ...
  const chartData = useMemo(() => {
    ...
    for (const code of districts) {
      const rate = index.get(code)?.get(year) ?? null;
      if (rate == null) continue;
  ...
  useEffect(() => {
    ...
    const nationalRate = index.get('BG')?.get(year) ?? null;
    ...
    const option: EChartsOption = {
      ...
      xAxis: {
        ...
        min: 0,
        ...(nationalRate != null ? {
          max: (v: { max: number }) => Math.ceil(Math.max(v.max, nationalRate) * 1.15),
        } : {}),
      },
      yAxis: {
        type: 'category',
...
function DataTable({ index, allYears, isBg, latestYear }: {
  ...
  const allCodes = useMemo(() => {
    ...
    return order.filter(c => index.has(c));
  }, [index]);

  const rows = useMemo(() => {
    const result: Array<{ code: string; name: string; year: string; rate: number | null }> = [];
    ...
    for (const yr of yearsToShow) {
      for (const code of allCodes) {
        ...
        const rate = index.get(code)?.get(yr) ?? null;
        if (rate == null) continue;
        result.push({ code, name: regionName(code, isBg), year: yr, rate });
      }
    }
    return result;
  }, [index, allYears, allCodes, yearFilter, levelFilter, isBg]);
