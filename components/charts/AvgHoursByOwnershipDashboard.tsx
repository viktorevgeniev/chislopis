...
export function AvgHoursByOwnershipDashboard({ data, dataset, locale = 'en' }: DashboardProps) {
  const isBg = locale === 'bg';

  const { quarters, totalByQ, privateByQ, publicByQ, latestQ } = useMemo(() => {
    const tMap: Record<string, number | null> = {};
    const priMap: Record<string, number | null> = {};
    const pubMap: Record<string, number | null> = {};
    ...
    data.forEach(row => {
      ...
      const code = row.Ownership_LFS_Code;
      qSet.add(q);
      if (code === 'total') tMap[q] = val;
      else if (code === '3') priMap[q] = val;
      else if (code === '4') pubMap[q] = val;
    });
  ...
  const kpi = useMemo(() => {
    ...
    const priv = privateByQ[latestQ];
    const pub = publicByQ[latestQ];
    const gap = priv != null && pub != null ? Math.round((priv - pub) * 10) / 10 : null;

    // QoQ change for total
    ...
    return { total, priv, pub, gap, qoqChange, prevQ };
  }, [latestQ, quarters, totalByQ, privateByQ, publicByQ]);
    ...
    <Card className="bg-slate-50 border-0 shadow-none">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-slate-500">
          {isBg
            ? `Тримесечни данни (${quarters[0]} – ${latestQ}) | Средни отработени часове седмично | По вид собственост`
            : `Quarterly data (${quarters[0]} – ${latestQ}) | Average actual weekly hours | By type of ownership`}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white shadow-sm rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              {isBg ? `Частен сектор (${latestQ})` : `Private Sector (${latestQ})`}
            </p>
            <p className="text-2xl font-bold mt-2 text-amber-600">
              {kpi.priv != null ? `${kpi.priv}h` : '—'}
            </p>
          </div>
          <div className="bg-white shadow-sm rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">
              {isBg ? `Публичен сектор (${latestQ})` : `Public Sector (${latestQ})`}
            </p>
            <p className="text-2xl font-bold mt-2 text-blue-600">
              {kpi.pub != null ? `${kpi.pub}h` : '—'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white shadow-sm rounded-xl p-4">
          ...
          <p className="text-xs text-slate-400 mb-3">
            {isBg ? 'Частен, публичен сектор и общо' : 'Private, public sector and total'}
          </p>
          <TrendLineChart
            quarters={quarters}
            totalByQ={totalByQ}
            privateByQ={privateByQ}
            publicByQ={publicByQ}
            locale={locale}
          />
        </div>

        {/* Chart B: Grouped Bar – Latest Quarter Snapshot */}
        <div className="bg-white shadow-sm rounded-xl p-4">
          <h3 className="text-sm font-semibold text-slate-700 mb-1">
            {isBg ? `Сравнение по сектори (${latestQ})` : `Sector Comparison (${latestQ})`}
          </h3>
          <p className="text-xs text-slate-400 mb-3">
            {isBg ? 'Частен vs Публичен сектор — последно тримесечие' : 'Private vs Public sector — latest quarter'}
          </p>
          <LatestQuarterBar
            latestQ={latestQ}
            privateVal={kpi.priv}
...
function TrendLineChart({ quarters, totalByQ, privateByQ, publicByQ, locale }: {
  quarters: string[];
  totalByQ: Record<string, number | null>;
  privateByQ: Record<string, number | null>;
  publicByQ: Record<string, number | null>;
  locale: 'bg' | 'en';
}) {
    ...
    const option: EChartsOption = {
      ...
      series: [
        ...
        },
        {
          ...
          type: 'line',
          data: quarters.map(q => privateByQ[q] ?? null),
        ...
        },
        {
          ...
          type: 'line',
          data: quarters.map(q => publicByQ[q] ?? null),
  ...
  }, [quarters, totalByQ, privateByQ, publicByQ, isBg]);
...
function LatestQuarterBar({ latestQ, privateVal, publicVal, totalVal, locale }: {
  latestQ: string;
  privateVal: number | null;
  ...
  useEffect(() => {
    ...
    const values = [privateVal ?? 0, publicVal ?? 0, totalVal ?? 0];
    const colors = ['#d97706', '#2563eb', '#1e293b'];
    ...
    window.addEventListener('resize', handleResize);
    return () => { window.removeEventListener('resize', handleResize); chart.dispose(); };
  }, [latestQ, privateVal, publicVal, totalVal, isBg]);
