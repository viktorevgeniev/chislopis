...
interface DivorcesDashboardProps {
  data: any[];
  dataset: Dataset;
  locale?: 'bg' | 'en';
}

export function DivorcesDashboard({ data, dataset, locale = 'en' }: DivorcesDashboardProps) {
  ...
  const kpiData = useMemo(() => {
    ...
    national.forEach(row => {
      ...
      const value = parseFloat(row.Divorces) || 0;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm font-medium text-muted-foreground">
                {locale === 'bg' ? `Бракоразводи (${kpiData.latestYear})` : `Divorces (${kpiData.latestYear})`}
              </p>
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm font-medium text-muted-foreground">
                {locale === 'bg' ? 'Градски бракоразводи' : 'Urban Share'}
              </p>
...
function RegionalChart({ data, availableYears, locale }: { data: any[]; availableYears: string[]; locale: 'bg' | 'en' }) {
  ...
  const regionalData = useMemo(() => {
    ...
    return filtered
      .map(row => ({
        ...
        value: parseFloat(row.Divorces) || 0
      }))
  ...
  useEffect(() => {
    ...
    const option: EChartsOption = {
      ...
      series: [{
        name: locale === 'bg' ? 'Бракоразводи' : 'Divorces',
        type: 'bar',
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="divorces-regional-year" className="text-sm font-medium">
            {locale === 'bg' ? 'Година' : 'Year'}
          </label>
          <Select
            id="divorces-regional-year"
            value={selectedYear}
  ...
  return (
    <div>
      <p className="text-sm text-muted-foreground mb-2">
        {locale === 'bg'
          ? 'Градска и селска съставка на бракоразводите'
          : 'Urban and rural composition of divorces over time'}
      </p>
